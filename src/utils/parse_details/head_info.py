...
def extract_head_info(soup: BeautifulSoup) -> Dict[str, Optional[str]]:
    ...
    title_tag          = wrapper.select_one("div.head-left h1")
    model_input        = wrapper.select_one("div.head-left input.ad-title-value")
    location_link      = wrapper.select_one("div.head-left .location a")
    updated_span       = wrapper.select_one("div.head-left .upped-at .arrow-up")
    posted_span        = wrapper.select_one("div.head-left .upped-at span:nth-of-type(2)")
    views_span         = wrapper.select_one("div.head-left .counters .views")
    favorites_span     = wrapper.select_one("div.head-left .counters .heart")
    price_usd_span     = wrapper.select_one("div.head-right .prices-block .main .price-dollar span")
    price_kgs_div      = wrapper.select_one("div.head-right .prices-block .main .price-som")
    price_rub_div      = wrapper.select_one("div.head-right .prices-block .addit .price-som:nth-of-type(1)")
    price_kzt_div      = wrapper.select_one("div.head-right .prices-block .addit .price-som:nth-of-type(2)")
